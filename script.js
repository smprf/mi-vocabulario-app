...
// ЗАМІНИ СТАРИЙ ОБРОБНИК НА ЦЕЙ dictionarySearch.addEventListener('input', () => { // Зберігаємо прапорець, що користувач скористався пошуком if (localStorage.getItem('dictionarySearched') !== 'true') { localStorage.setItem('dictionarySearched', 'true'); } filterDictionary(); // Викликаємо твою існуючу функцію фільтрації });
